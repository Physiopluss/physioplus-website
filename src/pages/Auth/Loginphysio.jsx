...
import { setOtpModalOpen } from "../../slices/modalSlice.js";
        ...
        onSubmit: (values) => {
            login(values.phone).then((res) => {
                if (res.status >= 200 && res.status < 300) {
                    toast.success(res.data.message, { id: "otpSendViaLogin", className: "capitalize" });
                    dispatch(setLoginData(values.phone));
                    setTimeout(() => {
                        dispatch(setOtpModalOpen());
                    }, 1500);
                } else if (res.status >= 400 && res.status < 500) {
                    toast.error(res.data.message, { id: "loginError1", className: "capitalize" });
                } else {
                    toast.error("Something went wrong", { id: "loginError2", className: "capitalize" });
                }
            });
        },
    });
