    ...
    const fetchTreatments = async () => {
      try {
        ...
        data.forEach((item) => {
          ...
          if (item?.isTreatmentScheduled?.isTreatmentCompleted) {
      ...
      <div className="px-3 py-4 space-y-3">
        {loading ? (
          ...
          ongoingTreatments.length ? (
            ongoingTreatments.map((item) => renderCard(item, false))
          ) : (
          ...
          completedTreatments.map((item) => renderCard(item, true))
        ) : (
          <p className="text-center text-gray-400 mt-4">
