...
import { Input, Button, Select, Option, Textarea, Radio, Checkbox, Breadcrumbs } from "@material-tailwind/react";
...
import { appointmentDataToRazorpay, cashAppointment, couponApi } from "../../api/booking";
...
const Booking = () => {
	...
	// google analytics
	useEffect(() => {
		ReactGA.send({ hitType: "pageview", page: window.location.pathname, title: "Booking" });
	}, []);
		...
		if (couponResponse?.status >= 200 && couponResponse?.status < 300) {
			amount != 0 && amount != null && amount != undefined && couponResponse
				? couponResponse?.data?.couponType == 0
					...
					: setAmountToPay(amount - (couponResponse?.data?.discount * amount) / 100)
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
						<div className="">
							...
							{formik.errors.patientName && formik.touched.patientName && (
								<span className="text-red-500 text-sm">{formik.errors.patientName}</span>
							)}
						</div>
						<div className="">
							<label
				...
				<div className="text-base sm:min-w-28 rounded-md gap-4 px-6 py-4 h-fit bg-white border border-gray-200 shadow-md lg:min-w-[350px] mb-8">
					...
					<div className="flex flex-col gap-2 sm:text-sm text-base">
						<p className="flex justify-between text-right gap-8">
							...
							{selectedDate && timeInString && moment(selectedDate).format("ll")}{" "}
							{!selectedDate && "Please select a date"}
						</p>
						<p className="flex justify-between capitalize">
							<span className="text-start">Booked Slot</span> {selectedDate && timeInString && timeInString}
						...
						{/* <p className="flex justify-between">
							<span className="text-nowrap">Day Treatment</span> ₹ 100
						</p>*/}
						<p className="flex justify-between">
							<span className="text-nowrap">Report Review</span> 0
						</p>
						...
						<div className={`${couponResponse ? "block" : "hidden"} `}>
							{couponResponse && couponResponse.status >= 200 && couponResponse.status < 300 ? (
								<div className="flex justify-between mb-2 capitalize">
									<span>Discount</span> <span className="text-green">- ₹ {amount - amountToPay}</span>
								</div>
							) : (
								<div className="text-red-600 mb-2 capitalize">{couponResponse?.message}</div>
							)}
						</div>
					</div>
					...
					<div className="text-wrap text-sm mt-2">
						...
						{/* <div className="flex gap-2">
							<MdPhoneInTalk className="w-8 h-8" />
							<p>Receive a 10% discount on your next session when you book within 7 days of your current treatment.</p>
						</div> */}
					</div>
				</div>
			</div>
