...
export default function SlotComponent({ physioData }) {
  ...
  useEffect(() => {
    if (selectedDate) {
      ...
      fetchBookedSlot(
        physioData?._id,
        moment(selectedDate).format("YYYY-MM-DDTHH:mm:ss.SSSSSS"),
        serviceSelected
      )
        .then((e) => {
  ...
  return (
    <div className="w-full mx-auto bg-white border rounded-lg p-4 shadow-md ">
      ...
      <Tabs value={activeTab}>
        ...
        <TabsBody>
          <TabPanel key={"home"} value={"home"} className="p-0">
              ...
              <div className="flex gap-2">
                ...
                >
                  {WorkingDaySlot.length === 0 ? (
                    <div className="text-sm font-semibold text-red-500 text-center">
                      No Slot Available
                    </div>
                  ) : (
                      ...
                      <SwiperSlide key={index} style={{ width: "auto" }}>
                        ...
                        >
                          <span className="text-sm">
                            {moment(slot).format("ddd")}
                          </span>{" "}
                          {/* Day (Mon, Tue, etc.) */}
                          <span className="text-lg font-medium">
                            {moment(slot).format("D")}
                          </span>{" "}
                          {/* Date (10, 11, etc.) */}
                        </div>
                      </SwiperSlide>
          ...
          <TabPanel key={"clinic"} value={"clinic"} className="p-0">
            <div className="flex flex-col gap-4 my-4 text-black">
              <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="w-full">
                  <Swiper
                    ...
                    freeMode={true}
                  >
                    {WorkingDaySlot.length == 0 ? null : timeSlots.length ==
                      0 ? (
                      <div className="text-sm font-semibold text-red-500 text-center">
                        No Slot Available
                      </div>
      ...
      {/* Continue Button */}
      <button
        className={`w-full py-1.5 rounded-full text-lg mt-2 
